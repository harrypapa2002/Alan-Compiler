...
llvm::Value *Let::igen() const
{
    ...
    llvm::Value *rValue = Builder.CreateLoad(rValueAlloc->getAllocatedType(), rValueAlloc);

    llvm::AllocaInst *lValueAlloc = llvm::cast<llvm::AllocaInst>(lexpr->igen());
    llvm::Type *lValueType = lValueAlloc->getAllocatedType();


    if (lValueType->isPointerTy())
    {
        llvm::Value *lPtr = Builder.CreateLoad(lValueType, lValueAlloc, "lPtr");
        Builder.CreateStore(rValue, lPtr);
    }
    else
    {
