...
llvm::Value *UnOp::igen() const
{
    llvm::Value *exprValue = expr->igen();
    llvm::AllocaInst *allocaExpr = Builder.CreateAlloca(exprValue->getType(), nullptr, "unop_tmp");
    if (llvm::isa<llvm::AllocaInst>(exprValue))
    {
        llvm::AllocaInst *exprAlloc = llvm::cast<llvm::AllocaInst>(exprValue);
        exprValue = Builder.CreateLoad(exprAlloc->getAllocatedType(), exprAlloc, "load_unop");
    }
    else
    {
        exprValue = Builder.CreateLoad(exprValue->getType(), exprValue, "load_unop");
    }
    ...
    {
    case '-':
        result = Builder.CreateNeg(exprValue, "negtmp");
        break;
    case '+':
        result = exprValue;
        break;
    ...
    Builder.CreateStore(result, allocaExpr);
    return allocaExpr;
}
