...
llvm::Value *StringConst::igen() const
{
    ...
    // Create the string as a constant data array
    llvm::Constant *strConstant = llvm::ConstantDataArray::getString(AST::TheContext, *name, true);
    ...
    llvm::Type *strType = strConstant->getType();
    llvm::ArrayType *arrayType = llvm::cast<llvm::ArrayType>(strType);
    uint64_t strLength = arrayType->getNumElements();

    // Allocate space on the stack for the string
    llvm::AllocaInst *alloca = Builder.CreateAlloca(arrayType, nullptr, "str_const_alloca");

    // Copy the global string constant into the allocated space
    Builder.CreateMemCpy(alloca, llvm::MaybeAlign(), globalStr, llvm::MaybeAlign(), strLength);
