...
{
    GenBlock *currentBlock = blockStack.top();
    llvm::AllocaInst *allocaInst = llvm::dyn_cast<llvm::AllocaInst>(currentBlock->getAlloca(*name));
...
llvm::Value *ArrayAccess::igen() const
{
    ...
    llvm::AllocaInst *arrayPtrAlloc = llvm::dyn_cast<llvm::AllocaInst>(currentBlock->getAlloca(*name));
...
llvm::Value *FuncDef::igen() const
{
    ...
    if (fpar)
    {
        ...
        for (auto &param : func->args())
        {
            ...
            currentBlock->addAlloca(*args[index]->getName(), &param);
        }
    }
    localDef->igen();
    stmts->igen();
