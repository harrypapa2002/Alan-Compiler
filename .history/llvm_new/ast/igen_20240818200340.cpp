...
llvm::Value *If::igen() const
{
    ...
    // Extend the condition value to i32 if necessary
    ...
    // Prepare the basic blocks for the then, else, and merge parts of the if statement
    ...
    // Create the conditional branch to then and else blocks
    ...
    thenStmt->igen();
    bool thenHasReturn = blockStack.top()->hasReturn();
    if (!thenHasReturn)
    {
        Builder.CreateBr(mergeBB);
    }
    thenBB = Builder.GetInsertBlock(); // Capture the final "then" block

    // Generate code for the "else" part (if exists)
    ...
    bool elseHasReturn = blockStack.top()->hasReturn();
    if (!elseHasReturn)
    {
        Builder.CreateBr(mergeBB);
    }
    elseBB = Builder.GetInsertBlock(); // Capture the final "else" block

    // If either then or else had a return statement, mark the merge block appropriately
